...
import { deletePGOwnerPermanently } from '@/app/actions/superadmin';
...
export async function GET(req: Request) {
  try {
    ...
    const isPGOwner = data?.type === 'pg_owner' || !!data?.ownerId;
    const isProperty = !isPGOwner && (data?.type === 'property' || !!data?.propertyId);
    const targetDashboardUrl = isPGOwner ? '/superadmin/owners' : isProperty ? '/pgowner/properties' : '/pgowner/tenants';
        ...
        <html>
          ...
          <body style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: #f8fafc; color: #1e293b; display: flex; justify-content: center; align-items: center; min-height: 100vh; margin: 0; padding: 20px; box-sizing: border-box;">
            <div style="background: white; border-radius: 16px; padding: 40px 30px; max-width: 480px; width: 100%; text-align: center; box-shadow: 0 10px 25px rgba(0,0,0,0.05); border: 1px solid #e2e8f0;">
              ...
              <p style="color: #64748b; font-size: 15px; line-height: 1.6; margin: 0 0 25px 0;">This security confirmation link has expired (5 minute limit). No changes were made.</p>
    ...
    if (isPGOwner) {
      ...
      const deleteResult = await deletePGOwnerPermanently(ownerId);
      if (!deleteResult.success) {
        return new NextResponse(`Failed to delete PG Owner: ${deleteResult.error}`, { status: 500 });
      }
      ...
      return new NextResponse(`
        ...
        <html>
          <head>
            <title>PG Owner Permanently Deleted - A1 Hostels</title>
            <meta name="viewport" content="width=device-width, initial-scale=1.0">
          </head>
          <body style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: #f8fafc; color: #1e293b; display: flex; justify-content: center; align-items: center; min-height: 100vh; margin: 0; padding: 20px; box-sizing: border-box;">
            <div style="background: white; border-radius: 16px; padding: 40px 30px; max-width: 480px; width: 100%; text-align: center; box-shadow: 0 10px 25px rgba(0,0,0,0.05); border: 1px solid #e2e8f0;">
              ...
              <h2 style="color: #0f172a; margin: 0 0 10px 0; font-size: 22px;">PG Owner Account Deleted</h2>
              <p style="color: #64748b; font-size: 15px; line-height: 1.6; margin: 0 0 25px 0;">The PG Owner account and ALL associated hostels, rooms, tenants, payments, and data have been permanently wiped from the database.</p>
              <a href="/superadmin/owners" style="display: inline-block; padding: 12px 28px; background: #4F46E5; color: white; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 15px;">Return to Super Admin</a>
            </div>
          </body>
        </html>
