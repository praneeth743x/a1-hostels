...
export default function DuesPage() {
  ...
  activeTenants.forEach(t => {
    if (!pendingDueTenantIds.has(t.tenant_id)) {
      ...
      combinedList.push({
        ...
        month: monthShort,
        dueDays,
        tenantStatus: t.is_active === false ? 'Vacated' : (t.status === 'notice_period' ? 'Notice Period' : 'Active'),
        isOverdue: false,
        isFullyPaid: false,
  ...
  const filteredList = combinedList.filter(item => {
     ...
     const matchesStatus = filters.status === 'All' || item.tenantStatus === filters.status;
